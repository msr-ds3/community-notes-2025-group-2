...
#check out ratings format
unzip -p ratings-00000.zip | head
#same format as notes!

#loop through rating files, extract and filter
for i in $(seq -w 0 19); do
    unzip -p ratings-000${i}.zip | awk -F'\t' 'NR == 1 || ($3 >= 1611360000000 && $3 <= 1627775999000)' > filtered_ratings_${i}.tsv
done

#manually moved it to folders for organization
#check number of lines in filtered_notes
